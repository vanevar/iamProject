...
@WebServlet(name="LoadEditFormServlet", urlPatterns={"/update"})
public class LoadEditFormServlet extends HttpServlet{
  @Inject
  IdentityDataService idDS;
  private static final long serialVersionUID = 1L;
  private static final Logger LOGGER = LogManager.getLogger(LoadEditFormServlet.class);
    ...
    if(uid != null && !uid.isEmpty())
    {
      ...
      if(errorString.isEmpty()) {
        try{
          ...
          req.setAttribute("formatedDate", dfm.stringFromDate(result.getBirthdate()));
          RequestDispatcher dispatcher = req.getServletContext()
              .getRequestDispatcher("/WEB-INF/views/editIdentity.jsp");
          dispatcher.forward(req, resp);
        }catch(Exception e)
        {
          LOGGER.error("Error ocurred when forwarding to the edit identity page. doGet method {}", e);
        }
      }
    }
    else
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    {
      LOGGER.error("Error ocurred when forwarding to doGet method {}", e);
    }
  }  
